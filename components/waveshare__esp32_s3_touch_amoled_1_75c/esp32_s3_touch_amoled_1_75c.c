...
#define BSP_LCD_DRAW_BUFFER_HEIGHT 10
...
esp_err_t bsp_touch_new(const bsp_display_cfg_t *cfg, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_touch_new_i2c_cst9217(tp_io_handle, &tp_cfg, ret_touch), TAG, "");

    const i2c_device_config_t direct_touch_config = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7,
        .device_address = ESP_LCD_TOUCH_IO_I2C_CST9217_ADDRESS,
        .scl_speed_hz = CONFIG_BSP_I2C_CLK_SPEED_HZ,
    };
    return i2c_master_bus_add_device(i2c_handle, &direct_touch_config, &touch_i2c_handle);
}
...
static lv_display_t *bsp_display_lcd_init(const bsp_display_cfg_t *cfg)
{
    ...
    const bsp_display_config_t disp_config = {
        .max_transfer_sz = BSP_LCD_H_RES * BSP_LCD_DRAW_BUFFER_HEIGHT *
    ...
    esp_lv_adapter_display_config_t disp_cfg = {
        ...
        .profile = {
            ...
            .buffer_height = BSP_LCD_DRAW_BUFFER_HEIGHT,
            /* Keep the small SPI draw buffer in internal DMA-capable RAM. Direct
             * PSRAM DMA shares MSPI bandwidth and underflowed with Wi-Fi active. */
            .use_psram = false,
...
#define CST9217_DATA_COMMAND 0xD0
#define CST9217_DATA_REGISTER 0x00
#define CST9217_DATA_ACK 0xAB
#define CST9217_DATA_LENGTH 10

static esp_err_t bsp_cst9217_touch_read(esp_lcd_touch_handle_t touch,
                                        ...
                                        void *user_ctx)
{
    ...
    const uint8_t reg[] = {CST9217_DATA_COMMAND, CST9217_DATA_REGISTER};
    const uint8_t ack[] = {CST9217_DATA_COMMAND, CST9217_DATA_REGISTER, CST9217_DATA_ACK};

    if (!touch || !points || !count || max_count == 0) {
        return ESP_ERR_INVALID_ARG;
    }
    *count = 0;

    esp_err_t ret = i2c_master_transmit_receive(touch_i2c_handle, reg, sizeof(reg),
                                                data, sizeof(data), 100);
    if (ret != ESP_OK) {
        return ret;
    }

    // CST9217 requires D0 00 AB after every data read. Without this handshake
    // the controller stops publishing subsequent samples.
    ret = i2c_master_transmit(touch_i2c_handle, ack, sizeof(ack), 100);
    if (ret != ESP_OK) {
        return ret;
    }

    if (data[6] != CST9217_DATA_ACK || (data[5] & 0x7F) == 0 || (data[0] & 0x0F) != 0x06) {
        return ESP_OK;
    }

    uint16_t x = ((uint16_t)data[1] << 4) | (data[3] >> 4);
    uint16_t y = ((uint16_t)data[2] << 4) | (data[3] & 0x0F);
    if (touch->config.flags.mirror_x) {
        x = touch->config.x_max - x;
    }
    if (touch->config.flags.mirror_y) {
        y = touch->config.y_max - y;
    }
    if (touch->config.flags.swap_xy) {
        const uint16_t temp = x;
        x = y;
        y = temp;
    }

    points[0].track_id = data[0] >> 4;
    points[0].x = x;
    points[0].y = y;
    points[0].strength = 1;
    *count = 1;
...
static lv_indev_t *bsp_display_indev_init(const bsp_display_cfg_t *cfg, lv_display_t *disp)
{
    ...
    touch_cfg.callbacks.custom_touch_read = bsp_cst9217_touch_read;
